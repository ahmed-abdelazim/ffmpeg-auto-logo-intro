...
for %%f in (*.mp4) do (
echo %%f
ffmpeg -i %%f -i ..\overlay.png -filter_complex "[1:v]scale=520:-1,format=rgba,colorchannelmixer=aa=0.5[fg];[0][fg]overlay=1380:920:enable='between(t,0,20)'" -r 30 -pix_fmt yuv420p -c:v libx265 -preset ultrafast -crf 22  -c:a libmp3lame ..\temp-%%f
